___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "displayName": "Adform Tracking",
  "description": "An unofficial template for Adform tracking.",
  "categories": ["AFFILIATE_MARKETING", "ADVERTISING"],
  "securityGroups": [],
  "id": "cvt_temp_public_id",
  "type": "TAG",
  "version": 1,
  "brand": {
    "thumbnail": "data:image/jpeg;base64,",
    "displayName": "",
    "id": "brand_dummy"
  },
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "displayName": "Mandatory Parameters",
    "name": "mandatoryParameters",
    "groupStyle": "NO_ZIPPY",
    "type": "GROUP",
    "subParams": [
      {
        "displayName": "Server of tracking point",
        "defaultValue": "track.adform.net",
        "simpleValueType": true,
        "name": "hostname",
        "type": "TEXT",
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          }
        ]
      },
      {
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          },
          {
            "type": "NUMBER"
          }
        ],
        "displayName": "Tracking setup ID of Adform client (pm)",
        "simpleValueType": true,
        "name": "pm",
        "type": "TEXT"
      },
      {
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          }
        ],
        "displayName": "Name of tracking point (pagename)",
        "simpleValueType": true,
        "name": "pageName",
        "type": "TEXT"
      }
    ]
  },
  {
    "displayName": "Optional Parameters",
    "name": "optionalParameters",
    "groupStyle": "ZIPPY_CLOSED",
    "type": "GROUP",
    "subParams": [
      {
        "displayName": "Divider",
        "defaultValue": "|",
        "simpleValueType": true,
        "name": "divider",
        "type": "TEXT"
      },
      {
        "notSetText": "Not set",
        "selectItems": [
          {
            "displayValue": "Order Object Variable",
            "value": "variable"
          },
          {
            "displayValue": "Order Object Table",
            "value": "table"
          }
        ],
        "displayName": "Order Definition",
        "simpleValueType": true,
        "name": "orderDropdown",
        "type": "SELECT"
      },
      {
        "help": "Provide the order object as the return value of a variable.",
        "enablingConditions": [
          {
            "paramName": "orderDropdown",
            "type": "EQUALS",
            "paramValue": "variable"
          }
        ],
        "displayName": "Order Object",
        "simpleValueType": true,
        "name": "orderVariable",
        "type": "TEXT"
      },
      {
        "help": "Provide the attributes you want to be included in the order object.",
        "enablingConditions": [
          {
            "paramName": "orderDropdown",
            "type": "EQUALS",
            "paramValue": "table"
          }
        ],
        "displayName": "Order Object",
        "name": "orderTable",
        "simpleTableColumns": [
          {
            "valueValidators": [
              {
                "type": "NON_EMPTY"
              },
              {
                "args": [
                  "^(sales|orderid|currency|basketsize|gender|agegroup|country|itms|var([1-9]|10)|sv(([1-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5]))|svn(([1-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])))$"
                ],
                "errorMessage": "Please provide a valid order variable",
                "type": "REGEX"
              }
            ],
            "defaultValue": "",
            "displayName": "Variable Name",
            "name": "name",
            "type": "TEXT"
          },
          {
            "valueValidators": [
              {
                "type": "NON_EMPTY"
              }
            ],
            "defaultValue": "",
            "displayName": "Variable Value",
            "name": "value",
            "type": "TEXT"
          }
        ],
        "type": "SIMPLE_TABLE",
        "newRowButtonText": "Add Variable"
      }
    ]
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

// Require the necessary APIs
const createQueue = require('createQueue');
const encodeUriComponent = require('encodeUriComponent');
const makeTableMap = require('makeTableMap');
const injectScript = require('injectScript');

// Get value of fields
const hostname = data.hostname;
const pm = data.pm;
const pageName = data.pageName;
const divider = data.divider;

// Determine whether or not the _adftrack array already exists
const _adftrackPush = createQueue('_adftrack');

// URL of Adform tracking point
const url = 'https://' + hostname + '/serving/scripts/trackpoint/async/';

// Data to be pushed into the Adform array
const requestData = {
  pm: pm,
  pagename: encodeUriComponent(pageName),
  divider: encodeUriComponent(divider)
};

switch(data.orderDropdown) {
  case 'variable':
    if(typeof data.orderVariable === 'object') {
      requestData.order = data.orderVariable;
    }
    
    break;
  case 'table':
    requestData.order = makeTableMap(data.orderTable, 'name', 'value') || {};
    
    break;
}

_adftrackPush(requestData);

injectScript(url, data.gtmOnSuccess, data.gtmOnFailure);


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "access_globals",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keys",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "_adftrack"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": false
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "inject_script",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urls",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "https://*.adform.net/serving/scripts/trackpoint/async/"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  }
]


___TESTS___

scenarios: []


___NOTES___

Created on 4.6.2019, 15:03:18


