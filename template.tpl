___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "displayName": "Adform Tracking",
  "description": "An unofficial template for Adform tracking, allowing more flexibility.",
  "categories": [
    "AFFILIATE_MARKETING",
    "ADVERTISING"
  ],
  "securityGroups": [],
  "id": "cvt_temp_public_id",
  "type": "TAG",
  "version": 1,
  "brand": {
    "thumbnail": "data:image/jpeg;base64,",
    "displayName": "peaksandpies",
    "id": "github.com_peaksandpies"
  },
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "name": "mandatoryParameters",
    "type": "GROUP",
    "subParams": [
      {
        "displayName": "Adform Server Domain",
        "defaultValue": "track.adform.net",
        "simpleValueType": true,
        "name": "hostname",
        "type": "TEXT",
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          }
        ]
      },
      {
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          },
          {
            "type": "NUMBER"
          }
        ],
        "displayName": "Adform Tracking ID (pm)",
        "simpleValueType": true,
        "name": "pm",
        "type": "TEXT"
      },
      {
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          }
        ],
        "displayName": "Tracking Point Name (pagename)",
        "simpleValueType": true,
        "name": "pageName",
        "type": "TEXT"
      }
    ]
  },
  {
    "type": "GROUP",
    "name": "order",
    "displayName": "Variables",
    "groupStyle": "ZIPPY_OPEN_ON_PARAM",
    "subParams": [
      {
        "type": "SELECT",
        "name": "orderVariable",
        "displayName": "Variables Object",
        "macrosInSelect": true,
        "selectItems": [],
        "simpleValueType": true,
        "notSetText": "Not set",
        "help": "Return an object which includes the order and product variables to be used in this tracking point."
      },
      {
        "type": "LABEL",
        "name": "variablesLabel",
        "displayName": "You can provide an object in the \"Variables Object\" field and also use the \"Variables Table\" field to manually override some variables."
      },
      {
        "type": "GROUP",
        "name": "orderTableGroup",
        "displayName": "Variables Table",
        "groupStyle": "ZIPPY_OPEN_ON_PARAM",
        "subParams": [
          {
            "help": "Provide the attributes you want to be included in the order object.\n\nReturn an object which includes the order and product variables to be used in this tracking point.",
            "enablingConditions": [],
            "name": "orderTable",
            "simpleTableColumns": [
              {
                "valueValidators": [
                  {
                    "type": "NON_EMPTY"
                  },
                  {
                    "args": [
                      "^(sales|orderid|currency|basketsize|gender|agegroup|country|itms|var([1-9]|10)|sv(([1-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5]))|svn(([1-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])))$"
                    ],
                    "errorMessage": "Please provide a valid order variable",
                    "type": "REGEX"
                  }
                ],
                "defaultValue": "",
                "displayName": "Variable Name",
                "name": "name",
                "type": "TEXT",
                "selectItems": []
              },
              {
                "valueValidators": [
                  {
                    "type": "NON_EMPTY"
                  }
                ],
                "defaultValue": "",
                "displayName": "Variable Value",
                "name": "value",
                "type": "TEXT"
              }
            ],
            "type": "SIMPLE_TABLE",
            "newRowButtonText": "Add Variable"
          }
        ]
      }
    ]
  },
  {
    "type": "GROUP",
    "name": "identifiersGroup",
    "displayName": "Extended Identifiers",
    "groupStyle": "ZIPPY_OPEN_ON_PARAM",
    "subParams": [
      {
        "type": "SELECT",
        "name": "identifiers",
        "displayName": "Extended Identifiers Object",
        "macrosInSelect": true,
        "selectItems": [],
        "simpleValueType": true,
        "notSetText": "Not set",
        "help": "Return an array which includes identifiers to be used in this tracking point."
      }
    ]
  },
  {
    "displayName": "Optional Parameters",
    "name": "optionalParameters",
    "groupStyle": "ZIPPY_OPEN_ON_PARAM",
    "type": "GROUP",
    "subParams": [
      {
        "displayName": "Divider",
        "defaultValue": "|",
        "simpleValueType": true,
        "name": "divider",
        "type": "TEXT"
      }
    ]
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

const callInWindow = require('callInWindow');
const createQueue = require('createQueue');
const encodeUriComponent = require('encodeUriComponent');
const getType = require('getType');
const injectScript = require('injectScript');
const makeTableMap = require('makeTableMap');
const templateStorage = require('templateStorage');

const hostname = data.hostname;
const pm = data.pm;
const pagename = data.pageName;
const divider = data.divider;
const orderVariable = data.orderVariable;
const orderTable = data.orderTable;
const identifiers = data.identifiers;

function assign(target) {
  if(getType(target) !== 'object') {
    return null;
  }
  
  const to = target;
  
  for(let index = arguments.length - 1; index > 0; index--) {
    const nextSource = arguments[index];
    
    if(getType(nextSource) === 'object') { 
      for(let nextKey in nextSource) {
        if(nextSource.hasOwnProperty(nextKey)) {
          to[nextKey] = nextSource[nextKey];
        }
      }
    }
  }
  
  return to;
}

const isEmptyObject = target => {
  if(getType(target) !== 'object') {
    return null;
  }
  
  for(let key in target) {
    if(target.hasOwnProperty(key)) {
      return false;
    }
  }
  
  return true;
};

const request = {
  pm: pm,
  pagename: encodeUriComponent(pagename),
  divider: encodeUriComponent(divider)
};

const variableData = getType(orderVariable) === 'object' ? orderVariable : {};
const tableData = getType(orderTable) === 'array' ? makeTableMap(orderTable, 'name', 'value') : {};
const order = assign({}, tableData, variableData);

if(getType(identifiers) === 'array') {
  request.eids = identifiers;
}

if(isEmptyObject(order) === false) {
  request.order = order;
}

if(templateStorage.getItem('loaded') === true) {
  callInWindow('adf.track', request.pm, request.pagename, request.order);
  data.gtmOnSuccess();
}
else {
  // https://s2.adform.net/banners/scripts/st/trackpoint-async.js
  const url = 'https://' + hostname + '/serving/scripts/trackpoint/async/';
  const _push = createQueue('_adftrack');
  
  _push(request);
  injectScript(url, () => {
    templateStorage.setItem('loaded', true);
    data.gtmOnSuccess();
  }, () => {
    templateStorage.setItem('loaded', false);
    data.gtmOnFailure();
  }, 'adf');  
}


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "access_globals",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keys",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "_adftrack"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": false
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "adf.track"
                  },
                  {
                    "type": 8,
                    "boolean": false
                  },
                  {
                    "type": 8,
                    "boolean": false
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "inject_script",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urls",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "https://*.adform.net/serving/scripts/trackpoint/async/"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "access_template_storage",
        "versionId": "1"
      },
      "param": []
    },
    "isRequired": true
  }
]


___TESTS___

scenarios: []


___NOTES___

Created on 4.6.2019, 15:03:18


